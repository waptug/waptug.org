...
The first decision the Groupe SpÃ©ciale Mobile made was to adopt a digital
...
\* Mobile Internet portal â€” a 3G portal that offers mobile access to content
...
in use â€” many different countries using different acronyms to refer to the same
thing.
...
The principles of data transmission â€” a refresher
...
modulation involve altering either the waveâ€™s amplitude or frequency.

The more 1â€™s and 0â€™s that can be interpreted from a single change will mean a
higher data rate.
...
This is known as Phaseâ€”Shift Keying (PSK).

Waves have relative starting points, or phases. Two identical waves, which have
different starting points, are said to be out of phase. If youâ€™re interested in
...
converge at the listening point they are in phase ..... or maybe itâ€™s just me!
...
not so long ago â€” you may recall the press coverage. The vast sums paid by
...
WAP â€” Wireless Application Protocol
===================================

http://www.wapforum.org

You may remember adverts on television urging viewers to â€œsurf the net, surf the
BT Cellnet."
...
subscriberâ€™s mobile device, due to the limitations of phone displays and
...
can be portrayed with just this simple level of display ï¬‚exibility.
...
I donâ€™t intend to go into too much detail about how encryption works, see my
guide to encryption, authentication and digital certiï¬cates for more information
on this subject.

Brieï¬‚y, to ensure that data being transmitted over the Internet cannot be
...
is run by the network operator, has been termed â€œthe gap in WAP" by some media,
...
WML provides support for applicationâ€”level security via the use of its SignText
...
The first modern digital computer, the ABC (Atanasoffâ€”Berry Computer), was built
...
two possible states â€” on or off. A switch that is on has value of 1, and a
switch that is off has a value of 0.

All information processed by a computer is a string of 1s and Us.

The smallest piece of information used by a computer is a bit â€” a single 1 or O.
...
outside world, be it a keyboard, mouse, ï¬‚oppy disk, CD, DVD, scanner or modem
for example.
...
printer, CDâ€”R, modem or sound card, for example.

Letâ€™s look at the principle components of a modern PC
...
There are two main types of motherboard â€” AT and ATX, the main difference
...
collectively as a â€œchip set." The image below shows an Intel Pentium CPU:
...
motherboard â€” the ROM BIOS. A ROM BIOS chip contains data that specifies the
...
be used in conjunction with CMOS chips which store userâ€”definable settings which
...
The BIOS is accessible when the PC first boots â€” you will normally see a line on
...
cycle." Aâ€ the switching activity in the computer occurs while the clock is
sending a pulse
...
system, which we wonâ€˜t worry about.)
...
physical (RAM) as well as virtual memory held in a â€œswap" or â€œpagingâ€ file on
the hard disk.
...
A role of the PC power supply is to â€œclean" the power supply as well as convert
...
IBM responded to the MCA design with a redeveloped version of ISA â€” EISA
...
Todayâ€™s preferred expansion technology is PCI (Peripheral Component
Interconnect). It uses a 32-bit bus and operates at around 33 MHz.

PCl uses bus-mastering technology which enables it to regulate the ï¬‚ow of data
over the data bus without the CPUâ€™s involvement.
...
As mentioned above PCl expansion cards are selfâ€”configuring â€” they determine the
...
To view a list of available and used I/O addresses you can use the â€œwinmsdâ€
program (or just â€œmsdâ€ if you running in DOS).
...
devices â€œtalk" at the same time.

Controlling the ï¬‚ow of data is called interruption. Every CPU has an interrupt
...
plug and play devices like PCI cards, but older â€œlegacyâ€ devices will need to be
configured manually.

The CPU therefore runs the BIOS, the Operating System, any applications that are
running as well as the HO addresses and lRQs of the PCâ€™s peripherals. This
results in the constant movement of a lot of data. Whilst this consumes a lot of
the CPUâ€˜s time, moving data is a relatively uncomplicated task and a waste of
the CPUâ€™s resources. Another chip, the DMA chip or 8237 chip, is used to control
...
likely conï¬‚ict with COM1 â€” the serial port. If you donâ€™t intend to use the
...
computer. It is the interface between the computerâ€™s hardware and the software
...
Itâ€˜s important to realize that not all computers have operating systems. The
...
neverâ€”changing hardware to control. For a computer like this, an operating
...
>   processor while downloading a ï¬le from the Internet while printing the text
...
Now letâ€™s look at the basic functions of an operating system.
...
firmware) will begin to activate the computerâ€˜s disk drives. In most modern
...
The operating systemâ€˜s tasks, in the most general sense, fall into six
categories:
...
Letâ€™s look at each of these categories in turn.
...
operating system hasnâ€˜t been carefully designed, the system can begin to use the
...
needs of the operating system itself. Letâ€˜s say that the operating system needs
...
to be relatively expensive. It also ignores a fundamental fact of life -â€” most
...
computerâ€˜s motherboard goes through a special program called a driver. Much of a
driver's function is as translator between the electrical signals of the
hardware subsystems and the highâ€”level programming languages of the operating
...
computers at a high speed even though there are times when the CPU canâ€˜t use
...
all the details in the CPU's operation. Letâ€˜s look at the example of creating a
...
There are also graphical user interfaces, such as Xâ€”Windows and Gnome, that make
Unix and Linux more like Windows and Macintosh computers from the user's point
of view.

Itâ€˜s important to remember that in all these examples the user interface is a
...
Now letâ€™s look at some of the common operating systems that have been developed
over the years.
...
became the de facto standard for PC development. Due to IBMâ€™s decision to
release the speciï¬cation into the public domain a wide range of â€œcompatibleâ€ PCs
developed by other manufacturers which complied to the same specification soon
arose.

DOS has three boot files â€” â€œio.sysâ€, â€œmsdos.sysâ€ and â€œcommand.comâ€
...
conï¬guration settings.

command.com is the â€œcommand interpreter". it provides what is known as the
shell. It is responsible for the handling of internal commands, commands that do
not have a â€œ.comâ€ or â€œ.exe" extension (such as copy, for example).
...
a .com or .exe extension matching the command. If it is unable to ï¬nd such a
file, it will return â€œBad command or file nameâ€.

Note â€” DOS also use another file, â€œconfig.sysâ€, which allows you to modify the
memory allocation and also specify an alternative to â€œcommand.comâ€ for the
shell.

These are the system files. Once the system has loaded, DOS then looks for a
file called â€œautoexec.bat". This file contains user-defined boot settings, the
equivalent of the startup folder in Windows.

DOS memory management

A PCâ€™s memory is divided into four distinct sections:
...
requires that each file has a 3â€”character extension.

### Windows 3.x

Windows 3.x was a multiâ€”tasking Graphical User Interface (GUI) which ran on top
of the DOS environment. Windows for Workgroups added limited support for
networking.

### Windows 9x (95, 98, 988E, ME)

Windows 95 was the first â€œself-containedâ€ Microsoft operating system that did
...
This allows manufacturers to write relatively â€œlightweight" drivers with only
...
introduction of ACPI â€” Advanced Configuration and Power Interface. This has the
goal of managing power more efficiently by enabling the OS to automatically
power devices on and off as required. But as you will know, this doesnâ€™t always
...
It therefore came in two ï¬‚avours: NT Server and NT Workstation.
...
32â€”bit operating system. Implemented in desktop and several server versions,
Windows 2000 focuses overall on improved ease of use, networking, management,
reliability, scalability, and security.

it comes in several ï¬‚avors:
...
3.5â€”inch floppy disks. Despite its userâ€”friendly features, the Macintosh lost
...
SMS (Short Message Service, or â€œtextingâ€ as it is more commonly known) has long
...
>   The ability to send SMS messages between networks â€” nationally and
>   internationally â€” was not possible until the late 1990s.
...
>   allow several messages to be â€œconcatenatedâ€ (joined together) allowing a
...
Note â€” the sender is still charged for sending 9 messages.

SMS messages can be received while a phone call is in progress, as all SMS
trafï¬c is sent via the GSM signalling channel. This enables the network to make
...
Launched in 2000, EMS was Ericssonâ€™s answer to Nokiaâ€™s Smart Messaging platform.
...
that messages could be personalised with emoticons â€” small graphics to express a
...
Delivery attempts are repeated at a preâ€”defined interval. Some devices allow a
â€œmessage validity" to be set, so that a message will expire if not delivered
...
culminated in the development of MMS â€” the Multimedia Messaging Service.

[image]

MMS will enable multiple forms of media to be combined in a single message
â€”text, graphics, animation and audio samples.

Later stages of the service will develop with the evolution of GSM as a whole
and allow for the inclusion of motion video.

This could be delivered in two ways â€” 30 seconds of motion video could recorded
with the terminal deviceâ€™s built-in camera and included in the body of a
message.
...
a new protocol â€” Wireless Session Protocol, or WSP.
...
MMSC â€” Multimedia Messaging Service Centre
...
pieces of hardware â€” the MMS Proxy Relay and the MMS Server. The Server stores
the message, the Proxy Relay forwards messages or sends message notifications to
terminal devices.

Application Gateway

The Application Gateway allows Internetâ€”based service providers to send content
...
The User Database holds information on the capabilities of a userâ€™s mobile
device and what services the user has subscribed to. Userâ€™s details are stored
...
the â€œwiredâ€ Internet, into WML format for interpretation by the â€œwireless"
browser on the userâ€™s handset.

MMS â€” from composition to delivery
...
MMS messages Mâ€ be created using a programming language optimised for the new
format, known as SMIL (Synchronised Multimedia Integration Language).

Pronounced â€œsmile", the language is similar in format to HTML and WML and
...
The orientation of the message can be specified also â€” landscape or portrait â€”
to best harness the screen resolution of the recipientâ€™s device.
...
The message itself will be â€œbuiltâ€ as follows:
...
>   The image for the ï¬rst slide

>   The image for the second slide

>   The text for the ï¬rst slide
...
â€œobject", guaranteeing efficient data transfer over the network as well as
compatibility with a wide range of both other mobile devices and also other
devices connected to the wired Internet.

[image]

To send the message the sender will need to enter the recipientâ€™s voice number
or email address.

The MMS message is then sent from the senderâ€™s mobile device to the networkâ€™s
MMSC using the WSP protocol.

The message is then stored at the MMSC and a notification message is then sent
to the recipientâ€™s terminal.

The message is sent using â€œPUSHâ€ technology. This was developed to enable WAP
content providers to send unsolicited messages to users without the userâ€™s
specific request.

To avoid receiving junk messages users can now specify from which gateways and
servers they wish to receive messages â€” unless the user has granted access to
the correct gateway then messages may be blocked.

Provided the recipientâ€™s terminal can receive an MMS message, it will send a
request message to the MMSC to download the message.

The MMSC then sends the message to the terminal, and once it receives
confirmation of the messageâ€™s receipt, it is removed from the message store.
...
Due to the "always on" nature of GPRS connections, it is the preferred â€œbearer"
...
1969 the entire intelligence and defence reï¬‚ex computer system was held in one
...
Networks can be conï¬ned to a room, a building or can be scattered across many
...
Internet is concerned it is one entity as it has a single â€domain". Taking WDS
...
voltages, for a copper cable network; pulses of light, for ï¬ber optic; or
...
A broadband network is the opposite of a baseband network â€” it can carry
...
where each party has to wait for the other to ï¬nish before transmitting. An
...
example a ï¬le server or a print server. A client is the computer that accesses
...
The OSI model deï¬nes the networking process and breaks it down into 7 layers.
...
>   They donâ€™t differentiate between applications, they are concerned with data
...
consists of fields containing information that is speciï¬c to the functions of
that protocol.
...
protocol. which typically adds both a header and a footer. The ï¬nal product,
...
Letâ€™s look at each of the layers in a little more detail.

The Physical Layer

At the bottom of the OSI model (or â€œprotocol stack") is the physical layer. This
...
computerâ€™s networking hardware and its networking software (in the network
...
on the network layer created the frame â€” a computer can have several different
...
protocols are what is known as â€œend-to-end" protocols. They are responsible for
...
datagramâ€˜s entire journey to the destination machine. This process is known as
routing.
...
There are two types of protocol in the transport layer â€” connection-oriented and
connection/ass.
...
Web browser to connect to an Internet sewer, the browser and the server ï¬rst
...
The presentation layer is responsible for determining what application the â€œraw"
data from the lower layers is destined for, and delivering it to the
corresponding port number (weâ€™ll look at ports in more detail later).
...
A name is a specific identiï¬cation of a machine, a user, or an application. It
is usually unique and provides an absolute target for the data. An address
typically identiï¬es where the target is located, usually its physical or logical
...
address consists of a 3-byte value called an organizationally unique identiï¬er
...
for a system on another network, the Destination Address ï¬eld contains the
...
Despite what I have just said above, although the word â€œInternetâ€ appears in
this protocolâ€™s name, it is not limited to use on the Internet, it can be used
...
ofï¬ce, with IP functioning as the envelope. The letter might be placed into
...
Now letâ€™s get a bit technical!
...
-   Version (4 bits) â€” the version of IP that created the datagram (the version
    in current use is 4, but version 6 is being developed).

-   IHL (Internet Header Length) (4 bits) â€” tells the receiving system the
    length of the IP header.

-   Type of Service (1 byte) â€” speciï¬es the priority for the datagram. Rarely
...
-   Total Length (2 bytes) â€” tells the receiving system the length of the entire
    datagram (in case it will need to fragment it).

-   ldentification (2 bytes) â€” this ï¬eld numbers the datagram so that the
    receiving machine can ensure datagrams are reassembled in the correct order.

-   Flags (3 bits) â€” used to regulate the fragmentation process.

-   Fragment offset (13 bits) â€” if a datagram is fragmented, this field
    identiï¬es the fragments place in the datagram for reassembly.

-   Time To Live (1 byte) - This ï¬eld specifies the number of networks that the
...
-   Protocol (1 byte) â€” This ï¬eld contains a code that identiï¬es the protocol
    that generated the information found in the Data ï¬eld.

-   Header Checksum (2 bytes) â€” This ï¬eld contains a Checksum value computed on
    the IP header ï¬elds only (and not the contents of the Data ï¬eld), for the
    purpose of error detection.

-   Source IP Address (4 bytes) â€” This ï¬eld speciï¬es the IP address of the
    system that generated the datagram.

-   Destination IP Address (4 bytes) â€” This ï¬eld speciï¬es the IP address of the
    system for which the datagram is destined.

-   Options (variable) â€” This ï¬eld is present only when the datagram contains
    one or more of the 16 available lP options. The size and content of the ï¬eld
    depends on the number and the nature of the options. Options include
    â€œtimestampingâ€ the datagram, allowing the IP header to increase to include
    the IP address of all routers it has passed through among others

-   Data (variable) â€” This ï¬eld contains the information generated by the
    protocol speciï¬ed in the Protocol ï¬eld. The size of the field depends on the
...
target machineâ€™s logical location on that network.
...
As mentioned earlier, IP addresses consist of two parts: a network identiï¬er and
a host identifier. All of the network interface adapters on a particular subnet
have the same network identiï¬er, but a different host identiï¬er. For systems
that are on the Internet, the network identiï¬ers are assigned by a body called
...
network, it is assigned a network identiï¬er, and it is then up to the network
...
network identifier and the host identiï¬er is not always in the same place. A
...
identiï¬er, depending on the size of the network.
...
the IP address have been carefully chosen to provide maximum ï¬‚exibility in
...
DHCP â€” Dynamic Host Control Protocol
...
doesn't speciï¬cally refer to it as such. On a Microsoft Windows 98 system, for
example, when you select the â€œObtain An IP Address Automaticallyâ€ radio button
...
client computer. The DHCP standard deï¬nes three types of IP address allocation,
...
detection, network layer protocol identiï¬cation, security, or anything else.
...
all of the information necessary to fulï¬l all of the functions described above.
...
more efï¬cient. Whilst it is necessary in Ethernet for every packet to contain
...
>   2. Link Establishment â€”Once the physical layer connection is established,
...
>   3. Authentication â€”If the two systems have agreed to the use of a particular
...
>   Data ï¬eld. Systems commonly use the Password Authentication Protocol (PAP)
>   or the Challenge Handshake Authentication Protocol (CHAP), but there are
>   other authentication protocols as well.

>   4. Link Quality Monitoring â€”If the two systems have negotiated the use of a
...
>   5. Network Layer Protocol Configuration â€”For each of the network layer
...
>   6. Link Open â€”Once the NCP negotiations are complete, the PPP connection is
>   fully established, and the exchange of packets containing network layer
>   application data can commence.

>   7. Link Terminationâ€” When the two systems have ï¬nished communicating, they
>   sever the PPP connection by exchanging LCP termination messages, after which
>   the systems return to the Link Dead state.

Summary â€” A datagramâ€™s life

You have now looked at how TCP constructs a datagram, and how IP is used to
route it to the destination machine â€” be it on an Ethernet network, or over the
Internet. Letâ€™s examine how a datagram passes along the Internet in a bit more
detail.
...
data, and then the IP header is constructed. Next, the ï¬rst hop (machine) of the
...
removes the outer header. The IP protocol on the network layer on the gatewayâ€™s
network interface calculates the checksum and veriï¬es the integrity of the
datagram. If the checksums don't match, the datagram is discarded and an error
message is returned to the sending device. Provided it does, next the TTL ï¬eld
...
hop of the route, either by analysis of the target address or from a speciï¬ed
...
performs a checksum calculation andâ€”assuming the two sums matchâ€”checks to see if
...
Itâ€™s a miracle it works at all!
...
When the Internet was ï¬rst â€œdeveloped", everyone thought that there were plenty
of addresses to cover any need. Theoretically, you could have 4,294,967,296
unique addresses (2â€). The actual number of available addresses is smaller
...
it requires modiï¬cation of the entire infrastructure of the Internet.
...
A Local Area Network uses IP addresses internally. Most of the network trafï¬c in
...
>   were not speciï¬cally allocated to that company by IANA (Internet Assigned
...
the value in the Destination Port ï¬eld and delivers the information in the Data
ï¬eld to the program or protocol associated with that port.
...
however. You can conï¬gure a Web server to use a port number other than 80, and
...
You can create a semi-secret Website of your own by conï¬guring your server to
...
File Transfer Protocol (FTP) enables a ï¬le on one system to be copied to another
...
copy one or more files to your machine. (The term transfer implies that the ï¬le
...
remote machines and transfers mail messages much like FTP transfers ï¬les. Users
...
accomplish this by using a distributed ï¬le system scheme. NFS systems are common
...
programming functions, return codes, and predeï¬ned variables to support
distributed computing.

Trivial File Transfer Protocol

Trivial File Transfer Protocol (TFTP) is a very simple, unsophisticated ï¬le
...
User Datagram Protocol (UDP) is a connectionlessâ€”oriented protocol, meaning that
...
subdivided into smaller networks. Because the network preï¬x is the same, the
...
Internetâ€™s global routing table.
...
>   3. How many hosts are there on the organisationâ€™s largest subnet today?

>   4. How many hosts will there be on the organisation's largest subnet in the
>   future?

The ï¬rst step in the planning process is to take the maximum number of subnets
required and round up to the nearest power of two. For example if a network
requires 9 subnets, 23 (=8) will not be enough subnet addressing space, so 2â€˜1
(=16) will be required.

The second step is to ensure that there are enough host addresses for the
organisationâ€™s largest subnet. If the largest subnet needs to support 50 host
...
Letâ€™s assume that an organisation has been assigned the network number 193.1.1.0
(Class C) by the Internetâ€™s governing body. The organisation decides it wishes
to deï¬ne 6 subnets. The largest subnet will need to support 25 hosts.

The ï¬rst step is to determine the number of bits required to define the six
...
is not possible to create an IP address that deï¬nes 6 subnets (as 6 is not a
...
Now hereâ€™s where it gets complicated!
...
assigned to the network number. The new â€œextended" network preï¬x (network number
...
Donâ€™t worry if that is as clear as mud! It makes more sense when you look at it
in binary notation.
...
This is a class C address, which means the ï¬rst three numbers deï¬ne the network
address, and the last number the host address on that network.

To create 8 subnets we are going to â€œborrow" three bits from the host address.
To identify that this is what we are doing we create a subnet mask:

11111111.1111111.11111111.11100000

This deï¬nes the three hits of the host address that are being used to create an
extended network preï¬x â€” now we only have 5 bits left to identify the host
...
With me so far? Weâ€™re not finished yet!
...
To identify the subnet, the administrator places the three-bit identiï¬er (the
last three bits in the bytes shown above) in the three-bit subnet identiï¬er we
...
The remaining 5 bits can be used to identify individual host addresses â€” up to
32 in total.
...
header is attached to this TCP â€œpacket" containing addressing details of the
...
>   between two parties only â€” it is not a multicast protocol.
...
TCP is a streaming protocol. Streaming implies that for every â€œwrite" action by
the sender, there is a corresponding "readâ€ action by the receiver. During the
establishment of the connection, the sender and receiver agree on the Maximum
Transmission Unit (MTU) â€” the maximum size of segment both parties can
comfortably handle. This value is regular updated by both parties during the
connection. Data is sent in a â€œstream" of packets.

TCP is also a rate adaptive protocol in that the rate of data transfer adapts to
the network load conditions. There is no pre-deï¬ned TCP data transfer rate; if
...
the Internet Protocol. TCP also uses its own header structure â€” it is the values
...
addresses. The next ï¬eld is a 32-bit sequence number, which identiï¬es the
...
ï¬eld contains six â€œï¬‚agsâ€ which contain information on the type of packet which
is being sent. These ï¬‚ags are single bits and have a value of 1 or 0 to indicate
on or off status.

>   the URG ï¬‚ag is used to indicate whether the packet is urgent or not

>   the ACK flag is used to indicate whether the packet is an ACK or not

>   the PSH ï¬‚ag is used if the sending system wants to â€œpushâ€ data to the remote
>   application

>   the RST flag is used to reset the connection

>   the SYN ï¬‚ag (for synchronise) is used during the establishment of the
>   connection

>   the FIN ï¬‚ag (for ï¬nish) is used to close the connection

following the data offset ï¬eld comes the Window ï¬eld which can contain an
optional amount of redundant data, up to 16 bits) to ensure that the preceding
ï¬elds all result in a header of the same size.
...
>   Maximum receive segment size (MTU) â€” this option is used when the connection
...
>   Window scale â€” this option is used to address the issue of latency (the
...
>   SACK-permitted â€” SACK stands for Selective Acknowledgment. Provided both
...
>   packets, the receiver only asks for the retransmission of packets it doesnâ€™t
>   receive (based on the segment numbers) or packets which are corrupted.

Connection establishment

The ï¬rst phase of a TCP session is the establishment of the connection. This
...
point of dynamic equilibrium between maximum network efï¬ciency and packet loss.
...
sender the size of its receiving buffer â€” this is the amount of data the
...
â€œsliding window" to determine the amount of data that can be placed on the
...
sender. The sender will not wait indeï¬nitely for an ACK packet, after a
predeï¬ned delay it will assume packet loss automatically and resend the
...
same time. A small ï¬‚ow of packets will be allowed to pass through, but a large
...
occurs again. Whilst all of the above sounds ï¬endishly clever, it is important
to note that there is no real intelligence as such â€” a limit is reached, it
...
selected packet. It is REDâ€™s function to signal the sender that there is a
potential for queue exhaustion. RED signals this likelihood of congestion by
adding a Congestion Experienced (CE) ï¬‚ag in the Type of Service (ToS) ï¬eld of
the 1P header.

Conclusion

TCP is not a predictive protocol. It is an adaptive protocol that attempts to
operate the network at the point of greatest efï¬ciency.
...
used today to protect valuable information â€” whether it is corporate information
held on intranets, or whether it is credit card information being transferred
across the Internet. The term cryptography is derived from the Greek kryptÃ©s
...
This guarantee of authenticity is important in todayâ€˜s reliance on secure data
...
detail at the use of digital certiï¬cates in both of these processes.
...
Confidentiality â€” assurance that only authorised users can read or use
conï¬dential information, and that to anyone who does manage to eavesdrop on the
transmission medium, the information will be unreadable.

Authentication â€” veriï¬cation of the identity of the entities that communicate
over the network.

Integrity â€” veriï¬cation that the original contents of information have not been
altered or corrupted, either deliberately or accidentally.

Nonrepudiation â€” assurance that a party involved in communication cannot falsely
...
An encryption algorithm is basically a very difï¬cult mathematical problem â€” a
problem so difficult that it is even difï¬cult for a computer to resolve.

What does it mean for a mathematical problem to be difï¬cult? To answer that let
me ï¬rst explain what is meant by an algorithm.
...
A problem is â€œdifficultâ€ if the fastest algorithm to solve the problem takes a
long time relative to the input size.

Factoring 15 to give 3x5 is a simple problem. The problem is more difï¬cult if
the numbers involved are signiï¬cantly larger but the algorithm remains the same.
Addition and factorisation are example of polynomial algorithms â€” the time taken
to resolve the problem is proportional to the size of the numbers involved.

Modern encryption methods rely on exponential algorithms â€” as the size of the
numbers involved increases, the time taken to resolve the problem increases
disproportionately.

To encrypt information, its value is adjusted by a pre-deï¬ned amount by applying
an algorithm to it. The amount by which the information is adjusted is speciï¬ed
...
Security has been gained at the price of veriï¬cation.
...
to the higher computational overhead â€” the processor has to do much more work to
decrypt the information.

When talking about the efï¬ciency of a cryptographic system, there are three
distinct factors to take into account:

Computational overheads â€” how much computation is required to perform the public
and private key transformations

Key size â€” how many bits are required to store the key pairs and any system
parameters

Bandwidth â€” how many bits must be communicated to transfer an encrypted message
or a signature
...
key encryption was ï¬rst proposed in 1975 by Stanford University researchers
Whitï¬eld Difï¬e and Martin Hellman.
...
128 to 160 bits in length and provide a digital identiï¬er for each ï¬le or
document.
...
considered the stronger of the two as it uses a longer key length â€” the longer
...
proceedings or ï¬nancial transactions, so digital signatures are used to identify
...
with the originatorâ€™s private key. This produces only a small amount of
...
such as the Internet trust relationships need to be veriï¬ed by a third party who
...
VeriSign) issues digital certiï¬cates. These certiï¬cates are used to sign public
and private encryption keys to prove that their identity is genuine.

If the CA is trusted, then any key that has been signed with a certiï¬cate issued
by that CA is also trusted.

Digital certiï¬cates function similarly to identiï¬cation cards such as passports:
...
The current industry standard for digital certiï¬cates is X.509 version 3. The
contents of a certiï¬cate is described in Appendix B.

Certificate servers

For use on an internal network you can use a certiï¬cate server and generate
digital certiï¬cates internally. Windows 2000 server has a certiï¬cate server
built in.

It is relatively easy to configure Internet Explorer to trust certiï¬cates issued
...
Click one of the following tabbed categories for the type of certiï¬cates you
want to install or remove:

-   Personal. Certiï¬cates in the Personal category have an associated private
    key. Information signed by using personal certiï¬cates is identified by the
    user's private key data. By default, internet Explorer places all
    certiï¬cates that will identify the user (with a private key) in the Personal
    category.

-   Other People. Certiï¬cates in the Other People category use public key
...
Depending on the version of SSL or TSL that the certiï¬cate was created using,
you may need to configure Explorerâ€™s security settings:
...
there are 24Â° possible values. By using a personal computer that can try 1
million keys per second, an attacker can try all possible keys in about 13 days.
However, an 128-bit key has 2â€˜25 possible values. If you could use a computer
...
A clearâ€”text authentication scheme used in Point-to-Point Protocol (PPP)
connections Password Authentication Protocol (PAP) is not a secure form of
authentication because the userâ€™s credentials are passed over the link in
...
merely identiï¬es the client to the PPP server; the server then authenticates the
...
authentication using smart cards, certiï¬cates and others. EAP, in conjunction
...
vendor-speciï¬c implementations of the CHAP standard.
...
>   username, along with an encrypted form of the serverâ€™s challenge, session
...
MS CHAP (Microsoftâ€™s implementation of CHAP)
...
in a format speciï¬cally designed for Windows NT and Windows 2000, and Windows 95
...
speciï¬cally for connecting to a server running Windows 95. You must use this
...
and receiving. To minimize the risk of password compromise during MSâ€”CHAP
exchanges, MSâ€”CHAP v2 drops support for the MSâ€”CHAP password change and does not
...
as network logon) from a Kerberos-enabled server, the client must ï¬rst contact
...
same machine.) The TGS gives the client a ticket that securely identiï¬es the
...
validate the clientâ€™s ticket without having to contact the domain Kerberos
...
Appendix B â€” Description of Digital Certificates
...
standardisation â€” IP Next Generation (an9) or IP Version 6 (IPv6).
...
protocols used by routers to handle IPv6 traffic. â€˜
...
>   IP conï¬guration either has to be done manually or with automatic
>   conï¬guration protocols such as DHCP (Dynamic Host Conï¬guration Protocol). As
>   more and more devices connect to the Internet conï¬guration needs to be
>   simpler.
...
This is achieved by moving non-essential and optional ï¬elds to extension headers
that are placed after the IPv6 header. This means headers are more efï¬ciently
...
To simplify host conï¬guration, IPv6 supports both stateful conï¬guration (in the
...
conï¬guration using network preï¬xes advertised by the router. Even in the absence
of a router hosts can conï¬gure themselves automatically without manual
conï¬guration.
...
New ï¬elds in the IPv6 header deï¬ne how trafï¬c is handled and identiï¬ed. Trafï¬c
identiï¬cation using a Flow Label ï¬eld in the header allows router to identify
and provide special handling for packets belonging to a ï¬‚ow (a series of packets
...
IPv6 addresses are separated into network preï¬x and host number in the same way
as IPv4 addresses.

There are three types of IPv6 address:

Unicast - a unicast address identiï¬es a single interface on the network

Multicast â€” a multicast address identiï¬es multiple interfaces on the network

Anycast â€” an anycast address identiï¬es all of the interfaces on the immediate
...
summarised to produce an efï¬cient routing hierarchy. A router running IPv6 only
needs to look at the ï¬rst few bits of the target address to know where to send
...
The TOP-Level Aggregation Identiï¬er is a 13-bit ï¬eld. This identiï¬es the highest
level in the routing hierarchy. These IDs are administered by the Internet
authority and allocated to local lSPs. This ï¬eld size allows for up to 8,192 TLA
le.
...
The Next-Level Aggregation Identifier is used to identify a speciï¬c customer
site. The ï¬eld is 24- bits. Together the TLA ID and the NLA ID form the public
part of the IP address.

SLA ID

The Site-Level Aggregation Identiï¬er is used by an individual organisation to
...
The Interface ID identiï¬es the interface (network card) on a specific subnet.

IPv6 Header

The IPv6 header is a ï¬xed size of 40 bytes:

[image]

Before we look at the changes in header structure in IPv6, letâ€™s look brieï¬‚y at
...
delivery through routers across the network. This ï¬eld is 8â€”bits and contains
...
payload). The size of this ï¬eld is 16-bits, which can indicate a maximum of
65,535 bytes.

Identification Identiï¬es the speciï¬c packet in a stream of packets. If a packet
is fragmented during transmission, all of the fragments retain the original
identiï¬cation value so that they can be grouped for reassembly.

Flags Identiï¬es flags for the fragmentation process. The size of this ï¬eld is 3
bits, however, only 2 bits are deï¬ned for use. There are two ï¬‚ags â€” one to
...
Protocol Identiï¬es the upper layer protocol. The size of field is 8 bits. For
example, TCP uses a protocol of 6, UDP 17 and ICMP 1.

Header Checksum Provides a checksum on the IP header. The size of this ï¬eld is
16 bits. The payload is not included in the checksum. Each node that receives
the packet veriï¬es checksum integrity and discards the packet if veriï¬cation
...
more lP options. The size of this ï¬eld is a multiple of 32 bits. If the IP
...
Traffic Class Indicates the class or priority of the packet. This ï¬eld provides
similar functionality to the va4 Type of Service ï¬eld.

Flow Label Indicates that this packet belongs to a speciï¬c sequence of packets
...
suCh as those needed by realâ€”time data (voice and video). For default router
...
before being discarded. The Hop Limit is similar to the IPv4 TTL ï¬eld.
...
header that must be processed by the router is the Hop-byâ€”Hop Options extension
...
The Next Header ï¬eld in the IP header indicates whether an extension is present
...
Extension Length ï¬eld, and an Options field that contains one or more options.
...
either intermediate destinations or the ï¬nal destination. It is used in two
ways:

If a Routing header is present, it specifies delivery or processing options at
each intermediate destination It speciï¬es delivery or processing options at the
ï¬nal destination

Routing header

Much like va4, va6 source nodes can use the Routing header to specify loose
routing. Alternatively speciï¬c routing information with the addresses of each
...
(see Jimboâ€™s Guide to Encryption, Authentication and Digital Certiï¬cates for
more information on this subject)

The Authentication header provides data authentication (veriï¬cation of the node
that sent the packet), data integrity (veriï¬cation that the data was not
modified in transit â€” either intentionally or unintentionally) and anti-relay
...
payload. To do this â€˜ it can be used in conjunction with the Encapsulating
...
(SPI) ï¬eld that identiï¬es the iPSec session information, as well as a Sequence
...
>   is typically sent when the packet is blocked by a ï¬rewall)
...
>   inability to resolve the destinationâ€™s link layer address
...
Discover addresses, address preï¬xes, and other conï¬guration parameters

ND is used by routers to:

>   Advertise their presence, host conï¬guration parameters, and on-link prefixes

>   Inform hosts of a better next-hop address to forward packets for a speciï¬c
>   destination
...
One of the most useful aspects of IPv6 is its ability to conï¬gure itself, even
...
can also determine the address of routers, other conï¬guration parameters,
additional addresses and link prefixes.

The address autoconï¬guration process for the physical interface of an IPv6 node
is as follows:

1.  A tentative link address is derived based on the link preï¬x

2.  Using duplicate address detection to verify the uniqueness of the tentative
    link address,a

>   Neighbour Solicitation message is sent with the Target Address ï¬eld that is
...
The next step of the conï¬guration process then continues:
...
This technology has many benefits â€” imagine being able to make a longâ€”distance
...
traditional PSTN are transmitted at a ï¬xed rate of about 64 kilobits per second
...
time, half of this transmitted data is wasted. If we could remove this â€œdead
...
>   Note â€” if you are making a dialâ€”up connection to the internet, you still pay
>   for the time you are connected and in this sense the connection is
>   circuit-switched, but the connection is only maintained with the lSPâ€™s
...
On a more complicated level, using VOIP, a companyâ€™s PBX becomes a gateway to
...
6.  A session is established between your company's PBX and the other partyâ€˜s IP
...
VOIP currently uses two main protocols â€” H232 and SIP. Both protocols are
...
data sharing and IP telephony. It was intended as an â€œall-in-one" solution to
...
untested and ever-changing as IP telephony a more ï¬‚exible protocol is required.

I wonâ€™t look at this protocol in any detail, but more information can be found
at
...
You can run Phone Dialer by typing â€œdialer.exeâ€ in the Run dialogue box on the
...
Select the option to make an Internet call and enter the recipientâ€™s IP address:
...
Currently VOIP does not offer sufï¬cient voice quality to be seriously considered
...
>   Determine the location of the target endpointâ€”SIP supports address
>   resolution, name mapping, and call redirection.

>   Determine the media capabilities of the target endpointâ€”Via Session
>   Description Protocol (SDP), SIP determines the â€œlowest level" of common
...
>   Determine the availability of the target endpointâ€”If a call cannot be
...
>   Establish a session between the originating and target endpointâ€”If the call
...
>   Handle the transfer and termination of callsâ€”SIP supports the transfer of
>   calls from one end point to another. During a call transfer, SIP simply
>   establishes a session between the transferee and a new endpoint (speciï¬ed by
...
>   Secondary Number â€” In

>   Secondary Number â€” Out
...
effectively three forms of server that can exist in the network â€” the SIP
...
of the called party. What will be available is perhaps an emailâ€”like address or
a

telephone number associated with the called party. Using this information, the
callerâ€™s user agent can identify with a specific server to "resolve" the address
...
The Session Description Protocol is responsible for negotiating a mutuallyâ€”
...
will be registered with the SIP server on the target userâ€™s network. Because
...
be speciï¬ed and the call will only proceed if this level can be guaranteed.
...
One of SIPâ€˜s advantages is its unique ability to return different media types
...
session with user Bob, whose URI is bgb\@biloxi.cgm. Aliceâ€™s UAC is configured
...
desires to invite Bobâ€™s UAS into a session (1): the server acknowledges the
request (2). Although Bobâ€™s UAS is identified by its URI, the outbound proxy
...
(the inbound server). Aliceâ€™s proxy server can now forward the INVITE message to
the inbound proxy server (5), which acknowledges the message (6). The inbound
proxy server now consults a location server to determine Bobâ€™s location (7), and
...
application accepts the call, Bobâ€™s UAS sends back an OK response to Alice
(13,14,15).

Finally, Aliceâ€™s UAC sends an acknowledgement message to Bobâ€™s UAS to confirm
...
endpoints have learned each otherâ€™s address from the INVITE/200 (OK) exchange,
...
was not available. Aliceâ€™s UAC can then issue a SUBSCRIBE message (1),
...
sends a NOTIFY message with Bobâ€™s current status of not signed in (7,8,9), which
Aliceâ€™s UAC acknowledges (10,11,12).
...
location service to reflect registration (2). The update is conï¬rmed by the
proxy (3), which confirms the registration to Bob (4). The PINT functionality
learns of Bobâ€™s new status from the location server (here we assume that they
are collocated) and sends a NOTIFY message containing Bobâ€™s new status (5),
which is forwarded to Alice (6,7). Aliceâ€™s UAC acknowledges receipt of the
notification (8,9,10).
...
format difference between the two types of messages is seen in the ï¬rst line.
The first line of a request has a method, defining the nature of the request and
a Request URI, indicating where the request should be sent. The ï¬rst line of a
...
>   ACK: conï¬rms reliable message exchanges
...
The combination of the To tag, From tag and Call-ID completely deï¬nes a
...
user agents. Whereas the Via header ï¬eld tells other elements where to send the
...
code and name. The lines that follow are a list of header ï¬elds. The Via, To,
From, Call-ID and CSeq header fields are copied from the INVITE request. (There
are three Via header field values â€” one added by Aliceâ€™s SIP UAC, one added by
